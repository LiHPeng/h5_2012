...
if(isset($_POST['checkname'])){
    ...
    $result=$conn->query("SELECT * FROM userinfo WHERE mobile = '$checkname'");//where后面跟的是数据库查询的条件。
...
if(isset($_POST['submit'])){//存在，说明前端已经点击了提交注册的按钮。
    $mobile=$_POST['phone'];//获取用户名
    $password=sha1($_POST['password']);//获取用密码
    $repass=sha1($_POST['repass']);//获取密码重复

    //将前端提交的数据存入数据库 - sql语句
    $conn->query("insert userinfo values(null,'$mobile','$password','$repass')");

    //后端跳转
    header('location:http://10.31.165.23/discount800/src/login.html');
